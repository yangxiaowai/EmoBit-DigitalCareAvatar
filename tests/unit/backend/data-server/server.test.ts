...
import { SqliteDataStore } from '@backend/data-server/sqliteStore.mjs';
...
describe('backend/data-server', () => {
    ...
    it('can select PostgreSQL storage for server deployments', async () => {
        const queries: string[] = [];
        const fakePool = {
            async query(sql: string) {
                queries.push(sql);
                if (sql.includes('SELECT elder_id AS "elderId"')) {
                    return { rows: [{ elderId: 'elder_demo' }] };
                }
                return { rows: [] };
            },
            async end() {
        ...
        const { server, store } = createDataServer({
            storage: 'postgres',
            rootDir: path.join(tempRoot, 'postgres-data'),
            legacyStatePath: path.join(tempRoot, 'missing-legacy.json'),
        ...
        expect(store.storageName).toBe('postgres');
        expect(healthzResponse.statusCode).toBe(200);
        expect(healthzResponse.json).toMatchObject({
            ok: true,
            storage: 'postgres',
        ...
        expect(queries.some((sql) => sql.includes('CREATE TABLE IF NOT EXISTS elder_state'))).toBe(true);
        await store.close();
    });
